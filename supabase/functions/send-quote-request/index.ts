...
Deno.serve(async (req) => {
  ...
  if (!rateLimitResult.allowed) {
    console.log("[send-quote-request] Rate limit exceeded for:", clientId);
    return new Response(
  ...
  try {
    ...
    if (parsed.data.honeypot) {
      console.log("[send-quote-request] Honeypot triggered - likely bot");
      return new Response(
        JSON.stringify({ success: true }), // Return success to fool bots
        { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
    
    const { image_paths, honeypot, ...data } = parsed.data;
