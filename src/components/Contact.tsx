    ...
    try {
      ...
      if (files.length) {
        ...
        for (const f of files) {
          ...
          const uploadRes = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/upload-quote-image`, {
            method: "POST",
            headers: {
          ...
          if (!uploadRes.ok || !uploadData.path) throw new Error(uploadData.error || "Upload failed");
          image_paths.push({ label: f.label, path: uploadData.path });
        }
      }
      ...
      setForm({ name: "", email: "", phone: "", type: "", domainChoice: "", domain: "", budget: "", honeypot: "" });
